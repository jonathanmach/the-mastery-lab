...
const allSchemas = ref<Array<{ id: string; title: string; base: string | null; own_field_count: number }>>([])


function closeMenu() { openRefPopoverId.value = null }
...
async function load() {
  error.value = ''
  try {
    allSchemas.value = await listSchemas()
  } catch (e) {
...
const treeData = computed<TreeNode[]>(() => {
  ...
  for (const s of allSchemas.value) {
    nodes[s.id] = {
      ...
      base: s.base,
      totalFields: s.own_field_count,
      ownFields: s.own_field_count,
      children: [],
    }
  }
    ...
    <div class="page-header">
      <div>
        ...
        <p class="page-header-meta">
          {{ allSchemas.length }} type{{ allSchemas.length !== 1 ? 's' : '' }} defined
        </p>
      </div>
