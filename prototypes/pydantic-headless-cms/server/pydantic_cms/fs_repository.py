...
class FileSystemSchemaRepository(ContentTypeRepository):
    """
    Implements ContentTypeRepository backed by .yml files.

    Each content type is stored as `{schemas_dir}/{id}.yml` in valid
    ...
    def save(self, schema: ContentTypeSchema) -> None:
        ...
        path = self._dir / f"{schema.id}.yml"
    ...
    def save_raw(self, schema_id: str, raw: dict) -> None:
        ...
        path = self._dir / f"{schema_id}.yml"
        path.write_text(yaml.dump(raw, sort_keys=False, allow_unicode=True))
    ...
    def exists(self, id: str) -> bool:
        return (self._dir / f"{id}.yml").exists()
...
def _content_type_to_json_schema(schema: ContentTypeSchema) -> dict:
    ...
    for field in schema.fields:
        if field.type == FieldType.list_:
            if field.item_type == FieldType.ref:
                if field.item_ref_schemas:
                    items = {"anyOf": [{"$ref": s} for s in field.item_ref_schemas]}
                else:
                    items = {"$ref": field.item_ref_schema}
                prop: dict = {"type": "array", "items": items}
            else:
                item_base = _FIELD_TYPE_TO_JSON.get(field.item_type, {"type": "string"}) if field.item_type is not None else {"type": "string"}
                prop = {"type": "array", "items": dict(item_base)}
        elif field.type == FieldType.ref:
            prop = {"$ref": field.ref_schema}
        else:
